...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and litecore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed litecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './litecore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in litecore-node services directory
