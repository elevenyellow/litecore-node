...
/**
 * Will return the path and default litecore-node configuration. It will search for the
 * configuration file in the "~/.garlicoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.garlicoin');
  var defaultConfigFile = path.resolve(defaultPath, './litecore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/garlicoind')
          }
        }
      }
    };
