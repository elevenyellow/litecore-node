...
var BitcoinRPC = require('garlicoind-rpc');
...
Bitcoin.DEFAULT_CONFIG_SETTINGS = {
  ...
  timestampindex: 1,
  spentindex: 1,
  zmqpubrawtx: 'tcp://127.0.0.1:242068',
  zmqpubhashblock: 'tcp://127.0.0.1:242068',
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of litecore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of litecore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of litecore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from litecore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of litecore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/garlicoind.pid';
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new BitcoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
        port: self.spawn.config.rpcport || 42068,
